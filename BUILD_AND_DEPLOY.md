...
src-tauri/target/release/bundle/
  nsis/NexusIDE_1.0.0_x64-setup.exe    <- Installer NSIS
  msi/NexusIDE_1.0.0_x64_en-US.msi    <- Installer MSI
```
...
4. `git add . && git commit -m "v1.0.0" && git push`
...
Releases → New → Tag: v1.0.0 → Upload les .exe/.msi → Publish
